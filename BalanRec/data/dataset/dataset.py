...
from BalanRec.utils.url import (
    decide_download,
    download_url,
    extract_zip,
    makedirs,
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, config):
        ...
        self._from_scratch()
        self.txt_embeddings = self.load_txt_embedding()
        self.nega_q_embeddings, self.item2nega_user = self.load_txt_embedding_black_list()
    ...
    def load_txt_embedding(self):
        """ get txt embedding """
        if int(self.config['TXT_EMBEDDING_FLAG']) == 1:
            heads = ['q', 'profile', 'dialogue']
            head_tgt = {'q': 'user', 'profile': 'item', 'dialogue': 'user'}
            embeddings = collections.defaultdict(dict)

            for head in heads:
                path = self.dataset_path + '/bert_embeddings/' + head + '_embeddings.json'
                with open(path, 'r', encoding='utf-8') as f:
                    embeddings[head] = json.load(f)
                    if head_tgt[head] == 'user':
                        field2id = self.field2token_id[self.uid_field]
                    else:
                        field2id = self.field2token_id[self.iid_field]
                    new_embeddings = dict()
                    for key in embeddings[head].keys():
                        if key in field2id.keys():
                            new_embeddings[field2id[key]] = embeddings[head][key]
                    embeddings[head] = new_embeddings
    ...
    def load_txt_embedding_black_list(self):
        if self.config['black_list_using']:
            path = self.dataset_path + '/bert_embeddings/' + 'nega_q_embeddings.json'
            inter = pd.read_csv(self.dataset_path + '/OMC-100k_nega.inter', sep='\t')
            item = pd.read_csv(self.dataset_path + '/OMC-100k.item', sep='\t')
            field2id = self.field2token_id[self.iid_field]

            item2nega_user = dict()
            for elem in list(item['item_id:token'].values):
                item2nega_user[field2id[str(elem)]] = \
                    list(set(inter[inter['item_id:token'] == elem]['user_id:token'].values))

            with open(path, 'r', encoding='utf-8') as f:
                embeddings = json.load(f)
            return embeddings, item2nega_user
        else:
            return -1, -1
    ...
    def _dataframe_to_interaction(self, data):
        ...
        for k in data:
            ...
            elif ftype == FeatureType.SINGLE_STR:
                toolfile = self.config.final_config_dict['data_path'] + '/' + k + '2id.txt'
                k2id = pd.read_csv(toolfile, sep='\t')
                k2id = dict(zip(k2id[k + ':single_str'].values, k2id['id:token'].values))
                value = [int(k2id[str(elem)]) if str(elem) in k2id.keys() else 0 for elem in list(value)]
                new_data[k] = torch.LongTensor(value)
            elif ftype == FeatureType.FLOAT:
